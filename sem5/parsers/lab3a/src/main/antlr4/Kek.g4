...
forArgsDecl returns [List<KekVar> kekVars]: (vars+=varTyped (COMMA vars+=varTyped)*)? {
    $kekVars = $vars.stream().map(x -> x.kekVar).collect(Collectors.toList());
    $kekVars.forEach(contextManager::addVar);
};
...
flowStatement returns [KekStatement kekStatement]:
    ...
    forStatement {$kekStatement = $forStatement.kekFor;} |
...
forStatement returns [KekFor kekFor]: FOR LPAR argsDecl+=forArgsDecl? SEMICOLON assign+=assignStatementNoEoln? SEMICOLON
                                      cond+=expression? SEMICOLON afterAssign+=assignStatementNoEoln? RPAR
                                      block[null] {
    $kekFor = new KekFor(
        $argsDecl.stream().map(x -> x.kekVars).findAny().orElse(null),
        $assign.stream().map(x -> x.kekAssign).findAny().orElse(null),
        $cond.stream().map(x -> x.kekExpr).findAny().orElse(null),
        $afterAssign.stream().map(x -> x.kekAssign).findAny().orElse(null),
        $block.kekBlock
    );
};
...
assignStatementNoEoln returns [KekAssign kekAssign]:
    varUsages+=varUsage (COMMA varUsages+=varUsage)* ASSIGN exprs+=expression (COMMA exprs+=expression)*
{
    if ($varUsages.size() != $exprs.size()) {
        throw new RuntimeException("Expected the same count of vars and expressions");
    }
